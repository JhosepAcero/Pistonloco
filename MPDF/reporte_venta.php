...
$query = "SELECT
    ...
    WHERE transacciones.transaccion_tipo = 'VENTA' AND transacciones.transaccion_id = '".$_GET['codigo']."';";
...
<div class="contenedor">
    ...
    <table id="tblFactura" align="center">
        ...
        $sql = "call SP_LISTAR_DETALLE_VENTA('".$row['venta_id']."')";
          ...
          while ($row2 = $resultado2->fetch_assoc()) {
            ...
            <tr>
                <td colspan="1" class="cantidad">'.$row2['detalleventa_cantidad'].'</td>
                <td colspan="1" class="precio">'.$row2['detalleventa_precio'].'</td>
                <td colspan="1">'.number_format((float)$row2['SUB_TOTAL'], 2, '.', '').'</td>
            </tr>';
        ...
        <tr class="detalle">
            ...
            <td colspan="1">';
            $importeQuince = (floatval((($row['venta_total'] - $row['venta_impuesto']) + $row['venta_descuento'])) - floatval($row['venta_impuesto'])); 
            $html .= '
            '. number_format((float)$importeQuince, 2, '.', '').'
            </td>
        </tr>
        <tr class="detalle">
            <td colspan="1" class="informacion">
